...
int udpserver(int argc, char *argv[])
{
    ...
    memcpy(&rsrc.sin_addr, host_ent->h_addr, host_ent->h_length);
    inet_pton(AF_INET, host_str, &(rsrc.sin_addr));


    /* get interface's name and address */
	if ( ifname[0] == '\0' && get_if_name() == -1 ) {
	...
	packet = malloc(IP_MAX_SIZE+linkhdr_size);
    ...
    while(running)
    {
		...
		/* Every 5 seconds, send a time exceeded packet */
		if (timeexc++ % 100 == 0)
		{
			send_icmp(icmp_sock, &rsrc, &dest_addr, (struct sockaddr_in*)0, 1);
		}

		ip = wait_packet(packet);
		if (ip)
		{
			sprintf(ips, "%d.%d.%d.%d", packet[linkhdr_size+12], packet[linkhdr_size+13], packet[linkhdr_size+14], packet[linkhdr_size+15]);
			memset(packet, 0, ip);
			...
			printf("send: %d\n", sendto(sock, ips, 1, 0, (struct sockaddr*)&sa, sizeof(struct sockaddr)));
		}
